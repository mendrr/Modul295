...
**Tomcat** — der **Webserver**, der in Spring Boot schon eingebaut ist. Er ist das Programm,
das im Hintergrund läuft und auf **Anfragen aus dem Browser/Internet wartet** (bei uns auf
**Port 8080**). Ruft später jemand `http://localhost:8080/teams` auf, nimmt Tomcat die Anfrage
an und reicht sie an deinen Controller weiter.
- *Woher?* Kam automatisch mit `spring-boot-starter-webmvc` mit — du musst ihn **nicht** selbst
  starten oder konfigurieren.
- Merksatz: **Tomcat = das Tor, durch das HTTP-Anfragen reinkommen.**
- Bild: der Pförtner am Eingang, der jeden Besucher (Anfrage) annimmt und zur richtigen Stelle schickt.

**Mapping** — heisst schlicht **Zuordnung / Übersetzung**: „A gehört zu B". Das gleiche
Grundprinzip taucht im Projekt an mehreren Stellen auf:
- **Objekt-Relational-Mapping (Klasse ↔ Tabelle)** — macht Hibernate: Klasse `Driver` ↔ Tabelle
  `drivers`, Feld `firstName` ↔ Spalte `first_name`.
- **Beziehungs-Mapping (Team ↔ Driver)** — deine `@OneToMany` / `@ManyToOne`: mappt die
  **Verbindung** zwischen den Tabellen. `mappedBy="team"` sagt, welches Feld die Beziehung trägt.
- **DTO-Mapping (Entity ↔ DTO)** — kommt als nächster Schritt: ein **Mapper** übersetzt zwischen
  Entity `Team` und dem `TeamDTO`, das die API nach aussen gibt (Muster aus 07A).
- **URL-Mapping (Adresse ↔ Methode)** — später mit `@GetMapping("/teams")`: mappt eine
  Web-Adresse auf eine Java-Methode.
- Merksatz: **Mapping = A wird B zugeordnet** — egal ob Klasse→Tabelle, Feld→Spalte, Entity→DTO
  oder URL→Methode.

**DTO (Data Transfer Object)** — ein einfaches „Nach-aussen"-Objekt, das **nur Daten trägt**
(z.B. als JSON an den Browser/Client). Es enthält bewusst **nur die Felder, die die API zeigen
soll** — ohne `@Entity`, ohne DB-Annotationen, ohne Beziehungs-Mechanik.
- *Warum nicht die Entity direkt rausgeben?*
  - **Kontrolle:** Du entscheidest, welche Felder sichtbar sind (keine internen/geheimen Felder).
  - **Trennung DB ↔ API:** Änderst du später die DB, muss sich die API nicht mitändern. Das DTO
    ist die stabile Aussen-Form.
  - **Endlosschleife vermeiden:** `Team` zeigt auf `Driver` und `Driver` zurück auf `Team` — bei
    direkter JSON-Ausgabe dreht sich das im Kreis. Deshalb hat `DriverDTO` bewusst **kein**
    `team`-Feld.
- Bild: Die Entity ist das volle Lager-Regal (alles drin, auch Internes). Das DTO ist das
  **Schaufenster** — nur das, was der Kunde sehen soll. Der **Mapper** ist der Mitarbeiter, der
  die Ware vom Regal (Entity) ins Schaufenster (DTO) stellt.
- Im Projekt: `TeamDTO` + `DriverDTO` = die Schaufenster-Versionen von `Team` + `Driver`.
...
### Schritt 8 — Docker starten, DB hochfahren, App zum ersten Mal starten
Erster echter Testlauf: Passen Entities, Repository und DB-Konfig zusammen?

**Was wir gemacht haben (in dieser Reihenfolge):**
1. **Docker Desktop gestartet** — der Docker-Daemon muss laufen, sonst kann kein Container hochfahren.
2. **DB-Container hochgefahren:** `docker compose up -d`
   - `up` = Container aus `docker-compose.yml` starten. `-d` = „detached", läuft im Hintergrund weiter.
   - Beim ersten Mal lädt Docker das Image `postgres:16` aus dem Internet (dauert einmalig).
   - Ergebnis: Container `f1-postgres` läuft, Postgres 16.14, Port `5432` nach außen offen.
3. **Geprüft, ob die DB bereit ist:** `docker exec f1-postgres pg_isready -U f1User -d f1db`
   - `exec` = einen Befehl **im** laufenden Container ausführen. `pg_isready` = Postgres-Werkzeug, das antwortet „accepting connections", wenn die DB Anfragen annimmt.
4. **App gestartet:** `./mvnw spring-boot:run`
   - `mvnw` = der mitgelieferte Maven-Wrapper (jeder auf dem Team hat so exakt dieselbe Maven-Version). `spring-boot:run` = kompiliert und startet die App.

**Was beim Start passiert ist (aus der Konsole):**
- Die App hat sich mit der DB verbunden (Hikari-Connection-Pool → `jdbc:postgresql://localhost:5432/f1db`).
- Wegen `ddl-auto=update` hat Hibernate **automatisch die Tabellen erzeugt** — genau das erwartete SQL:
  ```sql
  create table teams   (id bigint generated by default as identity, base varchar(255), name varchar(255), primary key (id));
  create table drivers (id bigint generated by default as identity, first_name varchar(255), last_name varchar(255),
                        number integer not null, points integer not null, team_id bigint, primary key (id));
  alter table if exists drivers add constraint ... foreign key (team_id) references teams;
  ```
  → Das ist der Beweis, dass `@OneToMany`/`@ManyToOne` und `@JoinColumn(name="team_id")` korrekt greifen: `drivers` hat die Spalte `team_id`, die auf `teams.id` zeigt.
- `Tomcat started on port 8080` + `Started F1teammanagerApplication` = App läuft, Webserver hört auf **Port 8080**.

**Gegengeprüft direkt in der DB:** `docker exec f1-postgres psql -U f1User -d f1db -c "\dt"`
→ zeigt beide Tabellen `drivers` und `teams`. ✅

✅ **Erledigt:** Docker + DB laufen, App erfolgreich zum ersten Mal gestartet, Tabellen automatisch angelegt.

> 💡 Nützliche Befehle für später:
> - App läuft im Vordergrund — mit `Strg + C` im Terminal stoppen.
> - DB-Container stoppen (Daten bleiben erhalten): `docker compose stop`
> - DB-Container wieder starten: `docker compose start`
> - In die DB schauen: `docker exec -it f1-postgres psql -U f1User -d f1db`

*(nächster Schritt: DTOs + Mapper — wie in 07A. Damit die API nicht die Entities direkt nach außen gibt.)*
